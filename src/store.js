...
const persist = config => (set, get, api) => config(args => {
  set(args)
...
const colors = c ? atob(c).split('') : [...new Array(36)].map((_) => {
  return Math.round(Math.random() * 4)
})

const wheelIndex = p <= MAX_INDEX ? parseInt(p, 10) : INIT_INDEX

export const useWheel = create(persist((set, get) => ({
  ...
  setIndex: (index, paletteIndex) => set({
